...
const {user1, loading} = useSelector(state => state.user)
const location = useLocation()
useEffect(() => {
  if(!loading && !user1) return;
  if(user1.role !== 'admin' && location.pathname === '/admin/products') {
    navigate('/access-denied')
  }
}, [user1, loading, location.pathname, navigate])
  ...
  return (
    ...
    <div className='grid grid-cols-4 gap-4 '>
      ...
      <div className='col-span-3 flex flex-col gap-5 max-lg:col-span-4'>
       <div className='flex items-center justify-between gap-5 max-[450px]:flex-col'> 
        <h1 className='text-2xl font-bold text-[var(--accent)]'>Products</h1> 
        {user === 'admin' && <button onClick={() => navigate('/add-product')} className=' py-2 px-4 rounded-lg flex items-center gap-2 max-sm:gap-1 max-sm:px-1 max-sm:text-sm'><Plus className='size-5 max-sm:size-4'/>Add Product</button>}
        <div  className='relative w-[250px] flex items-center justify-center max-lg:justify-between gap-2'>
            <p onClick={() => setShowFilters(!showFilters)} className='text-[var(--accent)] font-semibold hidden max-lg:flex items-center gap-2 p-3 rounded-full bg-gray-100'>Filter <ChevronDown className={showFilters ? 'rotate-180 transition-all duration-300' : 'transition-all duration-300'} size={20}/></p>
          <div onClick={() => setSortingOpen(!sortingOpen)} className='flex items-center bg-[var(--light-grey)] max-w-fit self-end rounded-full py-2 px-3 cursor-pointer hover:shadow-[0_0_10px_rgba(0,0,0,0.1)] shadow-amber-500'>
        ...
        {sortingOpen && <div className='flex flex-col gap-2 absolute top-[100%] right-0 bg-white p-3 shadow rounded-xl z-20 '>
