    ...
    <header className='w-full border-b border-gray-200 shadow sticky top-0 z-50 bg-white'> 
        ...
        <div className='flex items-center justify-between py-5 px-15  max-w-7xl mx-auto max-md:px-3'>
            ...
            <NavLink to={'/categories'} className='flex items-center border border-gray-200 p-3 rounded-full max-md:hidden cursor-pointer hover:shadow-[0_0_10px_rgba(0,0,0,0.2)] shadow-amber-600 transition-all duration-200'><LayoutDashboard className='inline mr-2 size-5'/> Categories</NavLink>
                ...
                {
                 user?.user?.role === 'admin' && <div onClick={() => setNavOpen(!navOpen)} className='flex items-center gap-3 p-3 rounded-full border border-gray-200 cursor-pointer hover:shadow-[0_0_10px_rgba(0,0,0,0.2)] max-md:hidden shadow-amber-600 transition-all duration-200'>Admin panel</div>   
                }
                { user?.user?.role === 'admin' && navOpen && <div className='absolute top-[100%] left-0 border border-gray-200 rounded-2xl flex flex-col gap-2 bg-white p-5'>
        ...
        <div className={`flex flex-col items-center justify-center max-[500px]:text-sm rounded-xl ${location.pathname === '/' ? 'bg-amber-100' : 'bg-white'} p-2`}><Home className='inline size-6' fill={location.pathname === '/' ? 'var(--accent)' : 'none'} onClick={() => navigate('/')} /><p className='max-[400px]:hidden'>Home</p></div>
            ...
            <div className={`flex flex-col items-center justify-center max-[500px]:text-sm rounded-xl ${location.pathname === '/products' || location.pathname === '/admin/products' ? 'bg-amber-100' : 'bg-white'} p-2`}><Shirt className='inline size-6' fill={location.pathname === '/products' || location.pathname === '/admin/products' ? 'var(--accent)' : 'none'} onClick={() => user?.user?.role === 'admin' ? navigate('/admin/products') : navigate('/products')} /><p className='max-[400px]:hidden'>Products</p></div>
           {user?.user?.role === 'admin' ? <div className={`flex flex-col items-center justify-center max-[500px]:text-sm rounded-xl ${location.pathname === '/admin/orders' ? 'bg-amber-100' : 'bg-white'} p-2`}><Package className='inline size-6' fill={location.pathname === '/admin/orders' ? 'var(--accent)' : 'none'} onClick={() => navigate('/admin/orders')} /><p className='max-[400px]:hidden'>Orders</p></div> : <div className={`flex flex-col items-center justify-center max-[500px]:text-sm rounded-xl ${location.pathname === '/cart' ? 'bg-amber-100' : 'bg-white'} p-2`}><ShoppingCart className='inline size-6' fill={location.pathname === '/cart' ? 'var(--accent)' : 'none'} onClick={() => navigate('/cart')} /><p className='max-[400px]:hidden'>Cart</p></div>}
            {user?.user?.role === 'admin' ? <div className={`flex flex-col items-center justify-center max-[500px]:text-sm rounded-xl ${location.pathname === '/admin/users' ? 'bg-amber-100' : 'bg-white'} p-2`}><User2 className='inline size-6' fill={location.pathname === '/admin/users' ? 'var(--accent)' : 'none'} onClick={() => navigate('/admin/users')} /><p className='max-[400px]:hidden'>Users</p></div> : <div className={`flex flex-col items-center justify-center max-[500px]:text-sm rounded-xl ${location.pathname === '/profile' ? 'bg-amber-100' : 'bg-white'} p-2`}><User2 className='inline size-6' fill={location.pathname === '/profile' ? 'var(--accent)' : 'none'} onClick={() => navigate('/profile')} /><p className='max-[400px]:hidden'>Profile</p></div>}
        </div>
    </header>
  )
}
