...
<div className='relative'>
  <input value={newComment} onChange={(e) => setNewComment(e.target.value)} type="text" placeholder='Write a review...' className='border border-gray-300 p-3 px-5 w-full max-sm:p-1 rounded-full pr-30 focus:outline-none focus:ring-2 focus:ring-[var(--accent)] transition-all duration-200' />
  <button onClick={addReview} className='absolute right-0 top-[50%] transform -translate-y-1/2 rounded-r-full h-full rounded flex items-center justify-center gap-4'><p className='max-sm:hidden'>Send</p> <Send size={24} className='rotate-45'/></button>
</div>
  ...
  <div className='my-5 p-5 bg-white rounded-2xl flex items-center gap-3 relative max-sm:flex-col' style={{backgroundColor: user?.username === review.username ? '#E0E7FF' : ""}} key={index}>
    ...
    <div className='flex flex-col gap-3'>
      <div className='flex items-center gap-3'>
        <div className='rounded-full w-15 h-15 hidden items-center justify-center bg-gray-50 max-sm:flex'><User size={20} className='mx-auto'/></div>
    ...
    {user?.username === review.username && <div className='absolute bottom-2 right-5 max-lg:static max-lg:flex max-lg:gap-3'><button onClick={()=> {setEditableReview(review.id); editableReview === review.id ? editReview(review.id) : ''}}>{editableReview === review.id ? 'Save' : 'Edit'}</button> <button onClick={()=> deleteReview(review.id)}>Delete</button></div>}
