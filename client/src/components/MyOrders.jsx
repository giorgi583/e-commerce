  ...
  return (
    <div className='max-w-7xl mx-auto py-15 flex-col gap-10 relative'>
        ...
        <div className='flex items-center justify-between text-lg mt-3'>
          <p className='text-2xl font-semibold text-[var(--secondary)]'>Total Amount: ${order.totalAmount}</p>
          <p>Order Status: {order.status}</p>
        </div>
        <button onClick={() => getOrderDetails(order.id)} className='flex items-center gap-2 mt-5'>{detailsOpen ? 'Hide Details' : 'View Details'} {!detailsOpen ? <ChevronDown/> : <ChevronUp/> }</button>
        {orderItems.length > 0 && detailsOpen && (<div className='mt-5'>
        <h2 className='text-2xl font-semibold text-[var(--secondary)]'>Order Items</h2>
        <div className='flex flex-col gap-5 mt-5'>
        {orderItems.map((item) => (
          <div key={item._id} className='flex items-center justify-between bg-indigo-50 py-4 px-10 rounded'>
